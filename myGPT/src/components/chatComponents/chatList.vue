...
const filterChatListOneDay = computed(() => {
    ...
    return todayChatList.value.filter(item => item.chatTitle.includes(search))
})
const filterChatListSevenDay = computed(() => {
    let search = searchInfo.value
    return weekChatList.value.filter(item => item.chatTitle.includes(search))
})
const filterChatListThirtyDay = computed(() => {
    let search = searchInfo.value
    return monthChatList.value.filter(item => item.chatTitle.includes(search))
})
...
function gettAllChatList() {
    ...
    // 向后端请求聊天记录,携带cookie
    axios.get('http://47.99.133.237:8080/user/getAllChatList', {
...
function distributechatList() {
    ...
    let tempTodayChatList = [];
    let tempWeekChatList = [];
    let tempMonthChatList = [];
    ...
    for (let i = 0; i < chatList.value.length; i++) {
        ...
        if (diff <= oneDay) {
            tempTodayChatList.push(chat);
        } else if (diff <= sevenDay) {
            tempWeekChatList.push(chat);
        } else if (diff <= thirtyDay) {
            tempMonthChatList.push(chat);
        }
    }

    todayChatList.value = tempTodayChatList;
    weekChatList.value = tempWeekChatList;
    monthChatList.value = tempMonthChatList;
}
...
function renameChat(ID) {
    ...
    ElMessageBox.prompt('请输入新的对话名称', '提示', {
        ...
        // 向后端发送重命名请求
        axios.post('http://47.99.133.237:8080/user/renameChat', {
            chatID: ID,
            chatTitle: value,
        },
            {
...
function deleteChat(ID) {
    ...
    ElMessageBox.confirm('此操作将永久删除该对话, 是否继续?', '提示', {
        ...
        // 向后端发送删除请求
        axios.post('http://47.99.133.237:8080/user/deleteChat', {
            chatID: ID,
        },
            {
                withCredentials: true,
...
<template>
    ...
    <el-scrollbar style="width:100%;" height="calc(100vh - 300px)">
        ...
        <div id="one-day-item">
            <span class="date-title">七天内</span>
            <div class="chat-item" v-for="item in filterChatListSevenDay" :key="item.chatID"
                @click="readChat(item.chatID)">
                <el-icon>
                    <ChatLineRound />
                </el-icon>
        ...
        <br>
        <div>
            <span class="date-title">三十天内</span>
            <div class="chat-item" v-for="item in filterChatListThirtyDay" :key="item.chatID"
                @click="readChat(item.chatID)">
                <el-icon>
                    <ChatLineRound />
                </el-icon>
