...
onMounted(() => {
    axios.get("http://47.99.133.237:8080/user/login", {
...
function logout() {
    // 携带cookie向后端发送请求
    axios.post("http://47.99.133.237:8080/user/logout", {}, {
        withCredentials: true
    }).then(res => {
        if (res.data.status === 'success') {
            ifLogin.value = false
...
function sendTypeQuestion() {
    ...
    // 携带cookie发送请求
    const source = new EventSource("http://47.99.133.237:8080/textQuestion?inputValue="
...
async function copyText(text) {
    try {
        // 非安全域下的复制
        //await navigator.clipboard.writeText(text)不可以，需要在安全域下
